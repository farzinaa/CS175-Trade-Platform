...
class tcn_agent(agent_thread):
    ...
    '''
    def __init__(self, moments = 30, batch_size = None, input_dim = 4 ):
    ...
    def _find_decision(self):
        ...
        if len(self.market_history) > self.moments:
            ...
            if not self.holding and predicted_value[1] - predicted_value[0] > 0:
                self.act = action.BUY
                self.holding = True
                self.holding_time = self.time_counter
                self.buy_in_price = self.market_history[self.time_counter - 1].price
                self.networth -= self.market_history[self.time_counter - 1].price
    ...
    def split_data(self, input, moments, lookahead = 1):
        ...
        for i in range(input.shape[0] - moments + lookahead):
