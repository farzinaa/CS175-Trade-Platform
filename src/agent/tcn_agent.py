...
class tcn_agent(agent_thread):
    ...
    def model(self, model):
        ...
        if model == '1': #report results, I tried this model with the default settings on the same dataset with
            ...
            x1 = TCN(return_sequences=False, nb_filters=(self.moments-1)*2, dilations=[1, 2, 4], nb_stacks=2, dropout_rate=.3,
                     kernel_size=2)(i)
            x2 = Lambda(lambda z: backend.reverse(z, axes=-1))(i)
            x2 = TCN(return_sequences=False, nb_filters=(self.moments-1)*2, dilations=[1, 2, 4], nb_stacks=2, dropout_rate=.1,
                     kernel_size=2)(x2)
            x = add([x1, x2])
            o = Dense(1, activation='linear')(x)

        elif model == '2':
            x1 = TCN(return_sequences=False, nb_filters=(self.moments - 1) * 2, dilations=[1, 2, 4], nb_stacks=2,
                     dropout_rate=.3,
                     kernel_size=2)(i)
            x2 = Lambda(lambda z: backend.reverse(z, axes=-1))(i)
            x2 = TCN(return_sequences=False, nb_filters=(self.moments - 1) * 2, dilations=[1, 2, 4], nb_stacks=2,
                     dropout_rate=.1,
                     kernel_size=2)(x2)
            x = add([x1, x2])
            x = LSTM(5, return_sequences=False, dropout=.3)(x)
            o = Dense(1, activation='linear')(x)
